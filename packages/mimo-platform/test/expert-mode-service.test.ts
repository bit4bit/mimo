...
function makeTmpDir(): string {
  const dir = join(tmpdir(), "expert-service-test-" + Math.random().toString(36).slice(2));
...
describe("ExpertService.readFileContent", () => {
  ...
  it("throws when file does not exist", async () => {
    await expect(service.readFileContent(workspace, "missing.ts")).rejects.toThrow("File not found");
  });

  it("rejects path traversal with ..", async () => {
    await expect(service.readFileContent(workspace, "../etc/passwd")).rejects.toThrow();
  });
});
...
describe("ExpertService.writeFileContent", () => {
  ...
  it("writes content to an existing file", async () => {
    ...
    const result = await service.writeFileContent(workspace, "hello.ts", "new content");

    expect(result.success).toBe(true);
    expect(readFileSync(join(workspace, "hello.ts"), "utf-8")).toBe("new content");
  });
  ...
  it("writes content to a file in a subdirectory", async () => {
    ...
    await service.writeFileContent(workspace, "src/utils.ts", "export const x = 1;\n");

    expect(readFileSync(join(workspace, "src", "utils.ts"), "utf-8")).toBe("export const x = 1;\n");
  });
