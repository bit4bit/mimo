...
describe("VCS Integration Tests", () => {
  ...
  beforeEach(async () => {
    testHome = join(
      tmpdir(),
      `mimo-vcs-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    );
  ...
  describe("Commit Flow VCS Methods", () => {
    ...
    describe("commitUpstream", () => {
      it("should use explicit commit message when provided for Git upstream", async () => {
        ...
        const result = await (vcs as any).commitUpstream(
          upstreamPath,
          "git",
          customMessage,
        );
    ...
    describe("Branch Operations", () => {
      describe("createBranch", () => {
        it("should create a new branch in Git repository", async () => {
          ...
          const result = await vcs.createBranch(
            "feature-branch",
            "git",
            upstreamPath,
          );
    ...
    describe("syncIgnoresToFossil", () => {
      it("should combine .gitignore and .mimoignore patterns into ignore-glob", async () => {
        ...
        writeFileSync(
          join(upstreamPath, ".gitignore"),
          ["# comment", "node_modules/", "", "dist/", "*.log"].join("\n"),
        );

        writeFileSync(
          join(upstreamPath, ".mimoignore"),
          ["# mimo-specific", ".mimo-cache/", "*.tmp"].join("\n"),
        );

        await vcs.createFossilRepo(repoPath);
        await vcs.openFossil(repoPath, agentWorkspacePath);

        const result = await vcs.syncIgnoresToFossil(
          upstreamPath,
          agentWorkspacePath,
        );
