...
import { createFileWatcherService, type FileWatcherService } from "../src/files/file-watcher-service";
...
describe("FileWatcherService", () => {
  ...
  describe("watchFile", () => {
    ...
    test("should detect file changes and emit outdated event", async () => {
      ...
      await watcher.watchFile("session-2", filePath, initialChecksum, (event) => {
        receivedEvents.push(event);
      ...
      await Promise.race([
        eventPromise,
        new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout waiting for event")), 3000))
    ...
    test("should not emit event when content checksum matches", async () => {
      ...
      const outdatedEvents = receivedEvents.filter(e => e.type === "file_outdated");
  ...
  describe("file deletion", () => {
    test("should emit file_deleted event when file is deleted", async () => {
      ...
      const deletedEvents = receivedEvents.filter(e => e.type === "file_deleted");
  ...
  describe("multiple sessions", () => {
    test("should isolate watches between sessions", async () => {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout waiting for events")), 3000))
      ]).catch(() => {});
