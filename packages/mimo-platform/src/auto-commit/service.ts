...
export class AutoCommitService {
  constructor(private readonly deps: AutoCommitDependencies) {}
  ...
  private async sync(sessionId: string): Promise<AutoCommitResult> {
    ...
    try {
      const { commitMessage, blocked, blockReason } =
        await this.generateCommitMessage(session);
      if (!commitMessage) {
        await this.setStatus(sessionId, {
          syncState: "idle",
          lastSyncError: undefined,
      ...
      if (blocked) {
        await this.setStatus(sessionId, {
          syncState: "idle",
          lastSyncError: blockReason,
        });
        return {
          success: false,
          message: blockReason || "Commit blocked due to high duplication",
        };
      }

      const result = await this.deps.commitService.commitAndPush(
        sessionId,
        commitMessage,
      );
      if (!result.success) {
      ...
      return {
        success: true,
        message: result.message || "Changes committed and pushed successfully!",
      };
    } catch (error) {
      const errorMessage =
