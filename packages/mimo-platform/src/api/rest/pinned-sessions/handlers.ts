 ...
 * GET /api/internal/users/:userId/pinned-sessions
 */
export async function listPinsHandler(
  ...
  const entries = await ctx.repos.pinnedSessions.list(user.username);
...
/**
 * Delete a pin by sessionId.
 * DELETE /api/internal/users/:userId/pinned-sessions/:sessionId
 */
export async function deletePinHandler(
  ...
  const ctx = c.get("mimoContext");
  await ctx.repos.pinnedSessions.remove(user.username, sessionId);
