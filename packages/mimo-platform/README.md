...
By default `PLATFORM_URL` resolves to `http://<MIMO_HOST>:<PORT>`, which works when the agent and platform run on the same machine. When they run on different hosts, set `MIMO_HOST` (or `PLATFORM_URL` directly) to the address the agent can reach:

```bash
MIMO_HOST=192.168.1.10 bun run index.ts
# Or explicitly set the full URL:
