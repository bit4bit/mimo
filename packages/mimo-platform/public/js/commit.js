...
(function() {
  ...
  async function fetchPreview() {
    ...
    try {
      ...
      // Show empty state
      commitTree.innerHTML = '<div class="commit-empty-state">Failed to load changes</div>';
    }
  }
  ...
  function updateUI() {
    ...
    const selectedVisible = visibleFiles.filter(f => selectedPaths.has(f.path));
  ...
  function renderTree() {
    if (!previewData || previewData.files.length === 0) {
      commitTree.innerHTML = '<div class="commit-empty-state">No changes to commit</div>';
      return;
    }

    const visibleFiles = getVisibleFiles();
    if (visibleFiles.length === 0) {
      commitTree.innerHTML = '<div class="commit-empty-state">No files match the selected filters</div>';
      return;
    }
      ...
      parts.forEach((part, index) => {
        ...
        if (!current[part]) {
          current[part] = {
            ...
            expanded: isLast ? expandedFiles.has(pathSoFar) : expandedDirs.has(pathSoFar)
          };
        }
  ...
  function renderTreeNodes(nodes, parentPath) {
    ...
    sorted.forEach(node => {
      ...
      if (node.type === 'file') {
        ...
        const selectedDescendants = descendantFiles.filter(p => selectedPaths.has(p));
      ...
      if (node.type === 'file') {
        ...
        if (node.file.isBinary) {
          ...
          statusBadge.textContent = node.file.status.charAt(0).toUpperCase() + node.file.status.slice(1);
        }
      ...
      if (node.type === 'file' && node.file.status === 'modified' && 
          !node.file.isBinary && expandedFiles.has(node.path)) {
        const diffEl = renderFileDiff(node.file);
        nodeEl.appendChild(diffEl);
      }
      
      // Make file labels clickable to expand diff
      if (node.type === 'file' && node.file.status === 'modified' && !node.file.isBinary) {
      ...
      if (node.type === 'directory' && node.expanded && Object.keys(node.children).length > 0) {
  ...
  commitBtn.addEventListener('click', async () => {
    ...
    commitTree.innerHTML = '<div class="commit-empty-state">Loading changes...</div>';
