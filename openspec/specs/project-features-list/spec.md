# project-features-list Specification

## Purpose

Projects accumulate planned work ("add OAuth", "dark mode") that today lives nowhere in the platform. This capability provides a lightweight per-project feature list where each feature is a `branchName` + plain-text description, plus a one-click path from a feature to a new session with the branch name prefilled and the description carried into the session notes.
