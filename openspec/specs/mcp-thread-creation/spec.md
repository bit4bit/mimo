# mcp-thread-creation Specification

## Purpose
TBD - created by archiving change spawn-thread-via-mcp. Update Purpose after archive.
## Requirements
...
- **THEN** the tool SHALL return an error result naming the invalid model, and SHALL
  NOT create a thread
- **AND** when the agent advertises models whose value or name is close to the
  requested value, the error SHALL suggest those close matches ("Did you mean: …")
  instead of listing every model
- **AND** when no close match exists, the error SHALL list the valid models
- **AND** the same validation and suggestion behavior SHALL apply to `mode`
