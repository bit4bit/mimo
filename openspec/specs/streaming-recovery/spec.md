...
When streaming events are rejected by the frontend's `shouldAcceptStreamingEvent` gate (due to null or mismatched `currentPromptId`), the system SHALL recover rather than silently dropping chunks. This ensures agent responses are always visible to the user.
