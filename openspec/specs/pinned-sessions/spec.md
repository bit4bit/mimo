...
Per-user, cross-project pinning of up to ~5 sessions with a global side-menu drawer and a `/pinned` parallel view rendering each selected pinned session in its own embedded iframe.
