# Specification: Diff Change Navigation

## Overview

Diff Change Navigation adds a change overview track and jump-to-change
keybindings to every scrollable diff surface (the PatchBuffer vertical-split
view and the per-file diffs in the commit dialog). A thin marker track renders
one colored tick per change hunk positioned proportionally to its location in
the file, with a viewport thumb; clicking a tick or pressing the configurable
`nextChange` / `previousChange` keys scrolls the next/previous hunk into view
and wraps around. A shared `MIMO_DIFF_OVERVIEW` browser module backs both
