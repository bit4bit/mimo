...
- [x] 1.1 Add failing integration test that session settings page shows all creation fields (Session Name, Assigned Agent, Agent working directory, Branch, MCP Servers, Session Type) with persisted values
- [x] 1.2 Add failing integration test that creation section is read-only (no editable controls for creation fields)
- [x] 1.3 Add failing integration test that fallback labels render exactly: `None`, `Repository root`, `Not set`, `None attached`
...
- [x] 3.1 Run `cd packages/mimo-platform && bun test` and confirm suite is green
