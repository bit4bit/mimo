...
- [ ] 2.1 Create `public/js/session-keybindings.js` with centralized key dispatcher
- [ ] 2.2 Wire actions to existing UI controls (`#create-thread-btn`, thread tabs, `#commit-btn`, notes textareas)
- [ ] 2.3 Add browser-safe matching (`event.key` + `event.code`) and scoped `preventDefault()`

## 3. Implement help overlay and auto-help

- [ ] 3.1 Add shortcuts help overlay markup and styling on session page
- [ ] 3.2 Add `Mod+Shift+/` toggle behavior and close controls
- [ ] 3.3 Add first-visit auto-help persistence via `localStorage`
- [ ] 3.4 Add hint UI for unrecognized `Mod+Shift+<key>` attempts

## 4. Integrate and verify

- [ ] 4.1 Load `session-keybindings.js` from session page layout
- [ ] 4.2 Ensure behavior works with chat, threads, commit, and notes scripts
