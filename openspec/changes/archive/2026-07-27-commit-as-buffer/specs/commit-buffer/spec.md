...
### Requirement: Commit modal and footer action buttons

The system SHALL no longer render a `#commit-dialog` modal or `#commit-btn`, `#sync-now-btn`, `#force-push-btn` buttons in the session footer bar. The commit review surface and all three actions are consolidated into the CommitBuffer.

#### Scenario: Footer bar no longer contains commit actions

- **WHEN** the session detail page renders
- **THEN** the footer bar contains no `#commit-btn` element
- **AND** the footer bar contains no `#sync-now-btn` element
- **AND** the footer bar contains no `#force-push-btn` element

#### Scenario: No commit modal in the DOM

- **WHEN** the session detail page renders
- **THEN** no element with id `commit-dialog` exists in the DOM