...
- **Trade-off**: Mode not persisted means the settings page can't say "this session is syncing branch X vs. created branch X."
  - **Acceptance**: Users care about _which branch_ the session uses, not how it got there. If that changes, adding a `branchMode` field to `Session` is a mechanical follow-up.
